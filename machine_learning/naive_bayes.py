...
def get_summary_total(summaries: Dict[str, List[Tuple]], label=None) -> int:
    ...
    :param label: The class label to count (if None, will return count of all labels)
    :return:
    """
    if label is None:
        total = 0
...
def split_by_class(dataset: List[Stock]) -> Dict[str, List[Stock]]:
    ...
    :return: A dict of lists, each list corresponding to a class
    """
    split = dict()
...
def summarise(dataset: List[Stock]) -> List[Tuple]:
    """
    Summarise the given dataset as a tuple
    :param dataset: The dataset to work on
    :return: A list of tuples in the format (mean, std deviation, length) for each attribute selected from each stock.
    """
...
def summarise_by_class(dataset: List[Stock]) -> Dict[str, List[Tuple]]:
    """
    Summarise the given dataset segmented by class.
